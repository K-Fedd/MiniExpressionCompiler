...
public class MiniExpressionCompilerFinal {
    ...
    private static int beFore(String expression) {
        ...
        sum = For(reverseNumbers, reverseOperators);
        return sum;
    }
    ...
    private static int For(Stack<Integer> nums, Stack<Character> ops) {
        ...
        while (lowerPrecedenceOperator.size() > 0) {
        ...
        reverseLowerPrecedenceOperator.push(lowerPrecedenceOperator.pop());
        while (higherPrecedenceOperator.size() > 0) {
        ...
        while (reverseHigherPrecedenceOperator.size() > 0) {
            reversePrecedenceOperatorsDupe = reversePrecedenceOperators;
            Stack<Integer> lowerPrecedenceHolder = new Stack<>();
            ...
            while (reversePrecedenceOperatorsDupe.peek() != '*' && reversePrecedenceOperatorsDupe.peek() != '/') {
            ...
            if (reversePrecedenceOperatorsDupe.peek() != '*' && reversePrecedenceOperatorsDupe.peek() != '/') {
                while (lowersPopped > 0)
                    lowerPrecedenceHolder.push(reverseLowerPrecedenceNumber.pop());
                ...
                reverseLowerPrecedenceNumber.push(sum);

                while (lowerPrecedenceHolder.size() > 0)
                    reverseLowerPrecedenceNumber.push(lowerPrecedenceHolder.pop());
            }
            else {
                ...
                reverseHigherPrecedenceNumber.push(sum);
            } 
        }

        while (reversePrecedenceOperatorsDupe.size() > 0) {
            int y = reverseLowerPrecedenceNumber.pop();
            int x = reverseLowerPrecedenceNumber.pop();
            char operator = reverseLowerPrecedenceOperator.pop();
        ...
        total = reverseLowerPrecedenceNumber.pop();
